...
import com.fiap.areslife.entity.SinaisVitais;
...
public class SaudeService {
    ...
    public SaudeHabitante criar(Long habitanteId, SaudeRequest request) {
        ...
        // Regra: status CRÍTICO dispara alerta automático
    ...
    public SaudeHabitante ultimoRegistro(Long habitanteId) {
        return saudeRepository.findTopByHabitanteIdOrderByDataRegistroDesc(habitanteId)
                .orElseThrow(() -> new ResourceNotFoundException("Nenhum registro de saúde encontrado para o habitante " + habitanteId));
    }
    ...
    private StatusSaude calcularStatus(SaudeRequest req) {
        ...
        if (sat.compareTo(new BigDecimal("90")) < 0
                || temp.compareTo(new BigDecimal("39.5")) > 0
                || freq > 130) {
            return StatusSaude.CRITICO;
        }

        if (sat.compareTo(new BigDecimal("95")) < 0
                || temp.compareTo(new BigDecimal("38.0")) > 0
                || freq > 100) {
    ...
    public List<SaudeHabitante> listar(Long habitanteId) {
        ...
        if (!habitanteRepository.existsById(habitanteId)) {
            throw new ResourceNotFoundException(
                    "Habitante não encontrado com id: " + habitanteId
            );
        }
        ...
        return saudeRepository.findById(id)
                .orElseThrow(() ->
                        new ResourceNotFoundException(
                                "Registro de saúde não encontrado com id: " + id
                        ));
    }
    ...
    public SaudeHabitante atualizar(Long id, SaudeRequest request) {
        ...
        saude.getSinaisVitais().setPressaoArterial(
                request.pressaoArterial());

        saude.getSinaisVitais().setFrequenciaCardiaca(
                request.frequenciaCardiaca());

        saude.getSinaisVitais().setSaturacaoOxigenio(
                request.saturacaoOxigenio());

        saude.getSinaisVitais().setTemperaturaCorporal(
                request.temperaturaCorporal());
